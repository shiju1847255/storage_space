...
<?php
			if(isset($_POST["btnSubmit"]))
			{
				...
				$KB = 1024;
				$totalBytes = $bytes * $KB;
				$ProUploadFolder = "ProUploadFolder";
				...
				foreach($_FILES["files"]["tmp_name"] as $key=>$tmp_name){
					...
					if(file_exists($ProUploadFolder."/".$name) == true){
						$UploadOk = false;
						array_push($errors, $name." file is already exist.");
					}
					
					if($UploadOk == true){
						$storeageID=$_SESSION['id'];
						$nnname=$storeageID."_".$name;
						move_uploaded_file($temp,$ProUploadFolder."/".$nnname);
						...
						foreach($uploadedFiles as $selected) {
							...
							$sql="UPDATE customer SET display_picture='$selected' WHERE customer_id='$storeageID'";

...
<div class="jumbotron" id="jumbotron">
	<div class="container">
	  <h1>Step 4</h1>
	  <p>Pictures and Payment Method</p>
	</div>
</div>
...
<div id="myForm" class="container" >
	...
	<div class="row">
		<div class="form-group">
					    ...
					    <label class="control-label">Select single or multiple files to upload :</label>
			...
			if(isset($_POST["btnSubmit"]))
			{
				...
				foreach($_FILES["files"]["tmp_name"] as $key=>$tmp_name){
					...
					if($UploadOk == true){
						...
						$nnname=$storeageID."_".$name;
						move_uploaded_file($temp,$UploadFolder."/".$nnname);
						array_push($uploadedFiles, $name);

						include 'dbh.php';

						foreach($uploadedFiles as $selected) {

							$sql="INSERT INTO storage_pictures(storage_id,picture_name) VALUES ('$storeageID','$selected')";
				...
				if($counter>0){
					...
					if(count($uploadedFiles)>0){
						...
						header("Location: RenterSpaceResult.php");
					}								
				}
				else{
...
<!-- end of php code -->
			<input class="btn btn-primary btn-sm pull-left" type="submit" value="Upload File" name="btnSubmit"/>
		</div>
	</form>
</div>
